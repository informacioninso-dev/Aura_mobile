...
const ESTADO_COLOR = { pendiente: '#F59E0B', parcial: '#60A5FA', pagado: colors.success }
    ...
    <GestureHandlerRootView style={[s.root, { paddingTop: topPad }]}>
      ...
      {totalPendiente > 0 && (
        <View style={s.totalBanner}>
          ...
          <Text style={[s.totalValue, { color: tab === 'me_deben' ? colors.success : colors.danger }]}>{formatMoney(totalPendiente)}</Text>
        </View>
      )}
      ...
      {loading
        ...
        : <FlatList
          ...
          renderItem={({ item }) => {
            const color = tab === 'me_deben' ? colors.success : colors.danger
...
const s = StyleSheet.create({
  ...
  totalBanner: { flexDirection: 'row', justifyContent: 'space-between', marginHorizontal: 16, marginBottom: 8, backgroundColor: colors.surface, borderRadius: 10, padding: 12 },
  totalLabel: { color: 'rgba(255,255,255,0.5)', fontSize: 13 },
  totalValue: { fontWeight: '700', fontSize: 15 },
  card: { flexDirection: 'row', backgroundColor: colors.surface, borderRadius: 12, padding: 14, borderWidth: 1, borderColor: colors.border },
  persona: { color: colors.text, fontWeight: '700', fontSize: 15 },
  ...
  saldo: { color: colors.textMuted, fontSize: 11 },

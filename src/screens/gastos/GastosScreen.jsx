...
const FIELDS_FIJO = [
  { key: 'descripcion', label: 'Descripción', type: 'text', placeholder: 'Ej: Arriendo' },
...
const FIELDS_PUNTUAL = [
  { key: 'descripcion', label: 'Descripción', type: 'text', placeholder: 'Ej: Supermercado' },
  { key: 'monto', label: 'Monto', type: 'number', placeholder: '0.00' },
  { key: 'categoria', label: 'Categoría', type: 'chips' },
...
export default function GastosScreen() {
  ...
  const cargar = useCallback(() => {
    setLoading(true)
    Promise.all([api.get('/finanzas/gastos-corrientes/'), api.get('/finanzas/gastos-no-corrientes/')])
  ...
  function abrirCrear() {
    const defaults = tab === 'fijos'
      ? { frecuencia: 'mensual', categoria: 'otro', fecha_inicio: new Date().toISOString().slice(0, 10), activo: true }
      : { categoria: 'otro', fecha: new Date().toISOString().slice(0, 10) }
  ...
  async function guardar(values) {
    ...
    const endpoint = tab === 'fijos' ? '/finanzas/gastos-corrientes/' : '/finanzas/gastos-no-corrientes/'
    try {
    ...
    Alert.alert('Eliminar', `¿Eliminar "${item.descripcion}"?`, [
      { text: 'Cancelar', style: 'cancel' },
      {
        text: 'Eliminar', style: 'destructive', onPress: async () => {
          const ep = tab === 'fijos' ? `/finanzas/gastos-corrientes/${item.id}/` : `/finanzas/gastos-no-corrientes/${item.id}/`
          await api.delete(ep)
          cargar()
        },
      },
  ...
  return (
    <GestureHandlerRootView style={s.root}>
            ...
            <SwipeableRow onEdit={() => setModal({ visible: true, item })} onDelete={() => eliminar(item)}>
              <View style={s.card}>
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={s.meta}>
                    {tab === 'fijos' ? (FREQ[item.frecuencia] || item.frecuencia) : item.fecha}
      ...
      <FormModal
        ...
        onSave={guardar}
        title={modal.item ? 'Editar gasto' : `Nuevo gasto ${tab === 'fijos' ? 'fijo' : 'puntual'}`}
        fields={fields}
